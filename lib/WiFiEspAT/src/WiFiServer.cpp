...
void WiFiServer::begin(uint8_t maxConnCount, uint16_t serverTimeout) {
  ...
  state = EspAtDrv.serverBegin(port, maxConnCount, serverTimeout) ? WIFI_LISTEN : WIFI_CLOSED;
}
...
void WiFiServer::beginSSL(bool ca, uint8_t maxConnCount, uint16_t serverTimeout) {
  ...
  state = EspAtDrv.serverBegin(port, maxConnCount, serverTimeout, true, ca) ? WIFI_LISTEN : WIFI_CLOSED;
}
